...
ADC_ChannelDef AdcChannelTable[ADC_CHANNEL_NUM] = {
		{ADC_CHANNEL_3, 	ADC_REGULAR_RANK_1, 	ADC2_15_PA3},
		{ADC_CHANNEL_18, 	ADC_REGULAR_RANK_2, 	ADC2_18_PA4},
		{ADC_CHANNEL_19, 	ADC_REGULAR_RANK_3, 	ADC2_19_PA5},
		{ADC_CHANNEL_5, 	ADC_REGULAR_RANK_4, 	ADC2_3_PA6},
		{ADC_CHANNEL_4, 	ADC_REGULAR_RANK_5, 	ADC2_5_PB1},
		{ADC_CHANNEL_15, 	ADC_REGULAR_RANK_6, 	ADC2_4_PC5},
};
#endif
...
{
	int32_t realData = MAX_TEMP;
	uint8_t i = 0;
	while(adcTable[i].adcRaw != MAX_ADC12)
	{
		if(raw < adcTable[i].adcRaw) {
			realData = adcTable[i].value;
			break;
		}
		i ++;
	}
