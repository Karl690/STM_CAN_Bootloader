...
void SetupMyLCD(SPI_LCD_HandleTypeDef* LcdHandler, int SPIIndex)
{
	...
	case 3:LcdHandler->SpiHandle.Instance = SPI3;
		...
		break;
#if defined(ST7735) || defined(ILI9341)
	case 4:LcdHandler->SpiHandle.Instance = SPI4;
		LcdHandler->Reset_Pin=GPIO_PIN_9;     LcdHandler->RESET_Port =GPIOE;
		LcdHandler->CS_Pin   =GPIO_PIN_11;    LcdHandler->CS_Port    =GPIOE;
		LcdHandler->RS_Pin   =GPIO_PIN_13;    LcdHandler->RS_Port    =GPIOE;
		LcdHandler->BKLT_Pin=99;              LcdHandler->BKLT_Port  =GPIOC;// no backlight control on spi4
		break;
#elif defined(ST7789)
	case 4:LcdHandler->SpiHandle.Instance = SPI4;
		LcdHandler->Reset_Pin=GPIO_PIN_9;     LcdHandler->RESET_Port =GPIOE;
		LcdHandler->CS_Pin   =GPIO_PIN_11;    LcdHandler->CS_Port    =GPIOE;
		LcdHandler->RS_Pin   =GPIO_PIN_15;    LcdHandler->RS_Port    =GPIOE;//E15 for embedded display E13 for spi4 display
		LcdHandler->BKLT_Pin=GPIO_PIN_15;     LcdHandler->BKLT_Port  =GPIOD;// no backlight control on spi4
		break;
#endif
...
//finished with the GPIO, now it is time to setup the spi port
  ...
  LcdHandler->SpiHandle.Init.NSS = SPI_NSS_HARD_OUTPUT;
 ...
 //////////////////// this part is for STM32H750 ///////////////////////////////////////////
  ...
  LcdHandler->SpiHandle.Init.FifoThreshold = SPI_FIFO_THRESHOLD_02DATA;
